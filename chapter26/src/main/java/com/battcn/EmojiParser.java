...
public class EmojiParser {
    ...
    public static final Pattern UNICODE_OUTLIERS = Pattern.compile("[\ud83c\udc00-\ud83c\udfff]|[\ud83d\udc00-\ud83d\udfff]|[\u2600-\u27ff]",
            Pattern.UNICODE_CASE | Pattern.CANON_EQ | Pattern.CASE_INSENSITIVE);

    public static final Pattern SPECIAL_CHARACTER = Pattern.compile("[`~!@#$%^&*()+=|{}':;',\\[\\].<>/?~！@#￥%……&*（）——+|{}【】‘；：”“’。，、？_]");
     ...
     * @return
     */
    public static String replaceAllEmoji(String source, String replacement) {
        if (source == null || "".equals(source)) {
            return source;
        }
        Matcher unicodeOutlierMatcher = UNICODE_OUTLIERS.matcher(source);

        return unicodeOutlierMatcher.replaceAll(replacement).trim();
    }
    ...
    public static String replaceAllSpecialCharacter(String source) {
        ...
        Matcher unicodeOutlierMatcher = SPECIAL_CHARACTER.matcher(source);
        return unicodeOutlierMatcher.replaceAll("").trim();
    }
    ...
    public static String filterEmoji(String source) {
        ...
        source = replaceAllSpecialCharacter(source);
    ...
    public final static String unicodeReg = "[" + "\u4E00-\u9FBF" + // ：CJK 统一表意符号 (CJK Unified Ideographs)
    ...
    public static String emojiChange(String string) {
        try {
            ...
            return replaceAllSpecialCharacter(sbBuffer.toString());
        } catch (Exception e) {
            e.printStackTrace();
        }
        return "";
